...
//  officerRole = g.roles.find("name", tag + " Officer");
  ...
  if(! officerRole.members.find("id", message.author.id)) {
    message.channel.send(`Error: You do not have the ${tag} Officer role!`, {code:"asciidoc"});
    return;
  }
...
//message.channel.send(`user: ${user}`, {code:"asciidoc"});
  ...
  if(!user) {
    message.channel.send(`Error: user ${username} is not in ${role}`, {code:"asciidoc"});
    return;
  }

  user.removeRole(role);
  message.channel.send(`User ${username} removed from ${tag}`, {code:"asciidoc"});
